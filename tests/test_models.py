...
def test_population_member_creation():
    ...
    assert member.quality_score == 0.85
...
def test_receipt_request_excludes_quality():
...
def test_receipt_response_approved():
    """TC-MOD-03: ReceiptResponse with decision='approved'."""
    ...
    assert resp.timestamp == 12.0
    assert resp.response_time == 2.0
    assert resp.was_corrected is False


def test_receipt_response_rejected():
    """TC-MOD-04: ReceiptResponse with decision='rejected'."""
    resp = ReceiptResponse(
        receipt_id="r-002",
        user_id="u-002",
        timestamp=18.0,
...
def test_sim_event_ordering():
    """TC-MOD-05: SimEvent comparison and sorting by time."""
